...
let STORY_WITHOUT_LIKES1 = STORIES.withoutLikes1;

import COMMENTS from  '../../fixtures/comment';
let COMMENT_WITHOUT_LIKES2 = COMMENTS.without_likes2;

let Like = app.models.Like;

const COLLECTION_URL = 'likes';

describe.only(`/${COLLECTION_URL}/create`, function () {
  updateTimeouts(this);

  it('Default post should be not found', () => {
    return user1Promise.then(({agent}) => {
      return agent.post(COLLECTION_URL)
        .send({subjectId: STORY_WITHOUT_LIKES1.id})
        .expect(404)
    })
  });

  it('Anonymous - deny to create like', () => {
    return api.post(`${COLLECTION_URL}/${STORY_WITHOUT_LIKES1.id}`)
      .expect(401)
  });
  ...
  it('User - allow to create like for a story', () => {
    return user1Promise.then(({agent}) => {
        return agent.post(`${COLLECTION_URL}/${STORY_WITHOUT_LIKES1.id}`)
          .expect(200)
          .expect(res => {
            let like = res.body;
            STORY_WITHOUT_LIKES1.id.should.eq(like.subjectId);
            'Story'.should.eq(like.subjectType);
          })
      })
      .then(() => Like.deleteAll({subjectId: STORY_WITHOUT_LIKES1.id, userId: user1.id}))
  });
